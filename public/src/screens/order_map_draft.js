...
function buildSection({ state, draft, order }) {
  ...
  // BD-ORDER-P-01 — pill copy uses natural Russian (no technical
  // request/draft/created/state wording leaking into the UI):
  //   state=success → ОПУБЛИКОВАН (green)
  //   state=missing → МАРШРУТ НЕ ВЫБРАН
  //   otherwise     → НОВЫЙ ЗАКАЗ (validation reuses the same pill)
  const label = order
    ? 'ОПУБЛИКОВАН'
    : (state === STATE.MISSING ? 'МАРШРУТ НЕ ВЫБРАН' : 'НОВЫЙ ЗАКАЗ');
