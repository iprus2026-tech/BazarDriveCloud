...
export function renderMelCard(mel = {}) {
  ...
  return [
    ...
    `Route: ${route}`,
    `File: ${file}`,
    `Role variant: ${mel.variant || '(whole screen)'}`,
    ...
    ``,
    `Recommended invariant guard (#684 R7 — the drafted static smoke pin for the recurring class)`,
    mel.recommendedGuard || '(none — set when the audit flags a recurring class)',
    ``,
