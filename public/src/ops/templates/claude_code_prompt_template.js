...
export function generateClaudeCodePrompt(screen = {}, mel = {}) {
  ...
  return [
    ...
    ``,
    `Step 0d — render-source, mount-timing & edge/ordering states (#684 #17 + R6 — classes that slipped past first push on this series)`,
    `A visual / behavioural change has failure modes a data-model or blast-radius check does NOT catch. Verify EACH before writing:`,
    ...
    `- edge / ordering states (#684 R6 — the dominant class Codex caught on the #732 series): if the change touches an overlay or an async / persisted write, walk the SECOND-ORDER states — a second overlay STACKED on top (does your trap consume Escape/Tab so it owns the keyboard and the one beneath defers? #738/#739); navigation-AWAY with the overlay still open (is the document-level trap / listener released so it never intercepts the next screen? #734); a PROGRAMMATIC write bypassing a native constraint (el.value = … skips maxlength; #749); a late / RETRY write appended OUT OF ORDER (insert by id, else a reload flips the thread; #750); a link to a record SEEDED LATER (does it hydrate before the record exists? #743).`,
    ``,
    `What to change`,
    repair,
    ``,
    ...
    `These are the mechanical defect classes that have each cost a review round on this screen series — all checkable BEFORE opening the PR:`,
    ...
    `- Edge / ordering (#684 R6): if you touched an overlay or an async / persisted write, re-confirm Step 0d's edge list — stacked-modal key ownership, trap release on navigate-away, native-constraint bypass (maxlength), out-of-order persist on retry, hydrate-before-seed. These were ~8 of the review rounds on the #732 series; catch them here, not in review.`,
  ].join('\n');
}
