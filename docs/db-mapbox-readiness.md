...
- **Mapbox stub foundation exists; the SDK-load/render seam is no longer dark by default.** `public/src/mapbox/` provides `map_shell.js` (`createMapShell()`), `driver_markers.js`, `trip_status_layer.js`, `geolocation_service.js`, `mapbox_config.js`, `mapbox_state.js`, `foundation_utils.js`, the SDK loader seam `mapbox_loader.js` (`loadMapboxSdk()` / `isMapboxSdkLoaded()` / `unloadMapboxSdk()`), and the route/price seam `route_service.js` (`estimateRoute`) / `price_estimator.js`. **`BD-MAP-FOUND-01` / `BD-MAP-ACTIVATE` (#805) landed since this baseline was written:** `mapbox_loader.js` now lazily loads the real vendored Mapbox GL JS (`public/vendor/mapbox-gl/`) whenever `mapbox_config.isMapboxEnabled()` is true (a token is configured — the committed production token is honored only on the GitHub Pages origin), and `public/src/screens/map.js` is the sole real `mapboxgl.Map` construction, consuming the loader and torn down by a router-owned disposer (`BD-SCREEN-LIFECYCLE-01A`, pinned by `scripts/smoke-screen-disposer-exactly-once.mjs` and `scripts/smoke-mapbox-render-map.mjs`). This is still per-surface: the other 7 `createMapShell()` consumers, and `/map` itself when dark or off the Pages origin, are unchanged — real rendering is not wired anywhere else, and the route/price seam (`route_service.js` / `price_estimator.js`) remains a stub with **zero callers**; `route_picker.js` still computes route/price with its own local mock (see BD-DOCS-035). The single `createMapShell()` contract is consumed by **8 screens**, not only the four map routes: `/map`, `/route-picker`, `/route-preview`, `/driver-map` **plus** `location_permission.js`, `order_map_draft.js`, `active_ride.js`, `active_ride_passenger.js` — all are on the same swap seam. `scripts/smoke-mapbox-foundation-stubs.mjs` pins "no `mapboxgl` / `api.mapbox.com` / `fetch` / external URLs", but its source-scan `MODULES` table currently covers only `driver_markers.js`, `trip_status_layer.js`, and `foundation_utils.js` — `map_shell.js`, `mapbox_loader.js`, `mapbox_config.js`, `geolocation_service.js`, `route_service.js`, and `price_estimator.js` are **not** scanned for forbidden tokens by that smoke (a gap M2 names) — the pricing module especially, since the real pricing API is meant to stay behind that seam; `mapbox_loader.js` / `mapbox_config.js` are a special case now, since they legitimately *do* reference `mapboxgl` / the token once enabled, so any such scan extension must account for that instead of flagging it as a violation. There is no `navigator.geolocation` call anywhere — geolocation is fully stubbed.
...
- `mapbox_loader.js` — `loadMapboxSdk()` / `isMapboxSdkLoaded()` / `unloadMapboxSdk()` is the established SDK-load seam; the real loader has **landed** (`BD-MAP-FOUND-01`, #805) and is a lazy loader for the vendored SDK (`public/vendor/mapbox-gl/`), dark (`Promise.resolve(null)`) whenever `isMapboxEnabled()` is false. Do **not** load Mapbox from `createMapShell()` or screens. **Resolved caveat:** `loadMapboxSdk()` now has exactly one caller, `public/src/screens/map.js` (`BD-MAP-ACTIVATE`, #805) — a screen-level call on `/map`'s own render path, gated behind `state === MAP_STATE.DEFAULT && isMapboxEnabled()`, not a shared app-level/bootstrap step. **Still open:** the other 7 `createMapShell()` consumers have no such caller yet, so this decision (screen-level call vs. a shared bootstrap/init step) has only been made for `/map`, not for the rest of the seam.
