...
- **localStorage key registry.** `public/src/storage_boundary.js` is the authoritative list of `bazardrive.*.vN` keys.
- **Mapbox stub foundation exists and is enforced.** `public/src/mapbox/` provides `map_shell.js` (`createMapShell()`), `driver_markers.js`, `trip_status_layer.js`, `geolocation_service.js`, `mapbox_config.js`, `mapbox_state.js`, `foundation_utils.js`. All four map screens (`/map`, `/route-picker`, `/route-preview`, `/driver-map`) render through the single `createMapShell()` contract. `scripts/smoke-mapbox-foundation-stubs.mjs` pins "no `mapboxgl` / `api.mapbox.com` / `fetch` / external URLs" in the stub sources and verifies SW precache of the foundation. There is no `navigator.geolocation` call anywhere — geolocation is fully stubbed.
...
Five mutation entry points, none request-id deduped today:
...
Already largely true. Extend smoke so the real-SDK swap stays isolated to ~5 points: the body of `createMapShell()` (`public/src/mapbox/map_shell.js`), `geolocation_service.js`, `mapbox_config.js` (token), and the hash-based route estimate in `public/src/screens/route_picker.js` (→ real Directions API). Screen render logic must stay untouched.

### Slice M3 — real geolocation behind the existing interface — planned
Replace the `geolocation_service.getPermissionStatus()` / `requestPosition()` stubs with real `navigator.*` calls. The `/location-permission` fork is already wired (`allow → /map?state=default`, `manual → /route-picker`, `back → /map`) and the real permission flow must lie on the same branch — do not collapse allow and manual.
