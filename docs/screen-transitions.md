...
| BD-ORDER-DETAIL-01 | `/order/<id>` | `public/src/screens/order_detail.js` | both | Done | 9 role-split states · `driver-send-offer` writes a DriverOffer and re-renders in place (toast); passenger select-driver writes overlay / selection and re-renders in place | Откликнуться / Принять (in-place re-render); explicit open-trip / open-active-ride CTA | in-place re-render; only the explicit open-trip / open-active-ride CTA navigates to `/active-ride?role=passenger&tripId=...` (or `/active-ride?role=driver&tripId=...&status=ACCEPTED`). Primary actions do not jump directly to `/respond` or `/trip-confirmation`. |
| BD-RESPOND-01 | `/respond?postId=...` (required; missing-state otherwise) | `public/src/screens/respond.js` | driver | Done | 1 driver-offer sheet | Отправить предложение/отклик | `/chat?responseId=resp_<post.id>&role=driver` (submit handler creates `responseId`; it is required to load the stored response thread). Bare `/chat?role=driver` would drop the context and fall back to the demo thread. Passenger-side response board remains `/responses`. |
...
- **BD-ORDER-DETAIL-01 primary actions** — `driver-send-offer` writes a `DriverOffer` and re-renders in place (toast); passenger select-driver writes overlay / selection and re-renders in place. Only an explicit open-trip / open-active-ride CTA navigates to `/active-ride?role=passenger&tripId=...` (or `/active-ride?role=driver&tripId=...&status=ACCEPTED`). Primary actions do not jump to `/respond` or `/trip-confirmation`.
- **BD-RESPOND-01 chat handoff** — submit handler creates `responseId = resp_<post.id>` and opens chat as `/chat?responseId=<responseId>&role=driver`. The `responseId` is required to load the stored response thread; a bare `/chat?role=driver` falls back to the demo thread.
