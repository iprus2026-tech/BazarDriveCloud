...
|---|---|---|---|---|---|---|---|
| BD-ORDER-DETAIL-01 | Order Detail | `/order/<id>` | `public/src/screens/order_detail.js` | Общий деталь-экран заказа, role-split passenger/driver. Runtime shell ship-нут в BD-ORDER-DETAIL-01C: `/order/<id>` зарегистрирован, экран рендерит все 10 состояний (P1–P4, D1–D4, S1, S2), Model B остаётся locked. Scoped local 01D writes landed: driver send/withdraw offer, passenger select-driver commit, passenger open-trip active_ride handoff, passenger cancel order, passenger reject offer, passenger cancel sent-offer sync, driver cancel accepted order. | Reads: deterministic demo fixtures из `order_detail.js`. Writes (scoped local, 01D landed): DriverOffer send/withdraw (`bazardrive.driver_offers.v1`); passenger commit (`bazardrive.order_overlay.v1` + offer.status sync); passenger open-trip active_ride handoff (`bazardrive.active_ride.v1`); passenger cancel order (overlay + sent-offer sync); passenger reject offer; driver cancel accepted order. No backend / no Mapbox / no fetch / no token / no CSP weakening. | **P0** | runtime shell present / Model B locked / 01D writes landed (см. issue #454, audit #455, contract #458, runtime #459) |
