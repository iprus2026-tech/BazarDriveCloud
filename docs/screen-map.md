...
|---|---|---|---|---|---|---|---|
| BD-ORDER-DETAIL-01 | Order Detail | `/order/<id>` (планируем) | `public/src/screens/order_detail.js` (планируем) | Общий деталь-экран заказа, role-split passenger/driver. Сейчас отсутствует runtime route — пассажир/водитель видят разные куски заказа в feed/respond/responses/chat без единой точки входа. Контракт зафиксирован в `screen-contracts.md` (BD-ORDER-DETAIL-01), Model B locked (BD-ORDER-DETAIL-01B). Рендер ещё не написан. | Reads: `mock_api` / планируемый `bazardrive.driver_offers.v1` (Model B DriverOffer store) / `bazardrive.active_ride.v1` после seed. Writes (future runtime, не в этом PR): driver «Откликнуться» → только `DriverOffer(status='sent')`; passenger «Выбрать водителя» → `Order.status='ACCEPTED'` + `selectedDriverId` + offer статусы + seed `bazardrive.active_ride.v1`. | **P0** | missing runtime / contract-gated / Model B locked (см. issue #454, audit #455, contract #458) |
