...
|---|---|---|---|---|---|---|---|
| BD-ORDER-DETAIL-01 | Order Detail | `/order/<id>` | `public/src/screens/order_detail.js` | Общий деталь-экран заказа, role-split passenger/driver. Runtime shell ship-нут в BD-ORDER-DETAIL-01C: `/order/<id>` зарегистрирован, экран рендерит все 10 состояний (P1–P4, D1–D4, S1, S2), Model B остаётся locked. Mutating Model-B actions (driver-offer create, passenger commit, cancel, reject) — non-mutating stubs в этом PR, отложены до BD-ORDER-DETAIL-01D. | Reads (runtime сейчас): deterministic demo fixtures из `order_detail.js`; маппинг на реальный mock_api появится в 01D. Writes: **none** — все CTA это `data-action` стабы с toast-уведомлением «Действие будет подключено в 01D» / «Оффер будет подключён в 01D». No backend / no Mapbox / no fetch / no token / no CSP weakening. | **P0** | runtime shell present / Model B locked / writes pending (см. issue #454, audit #455, contract #458, runtime #459) |
