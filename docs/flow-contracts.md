...
| `public/src/state.js`                            | User profile store (`bazardrive.user.v1`). Drives onboarding gating, driver readiness (`documentsReady` = registration docs `REGISTRATION_DOCS`; `shiftDocsReady` = all docs incl. waybill / medical; `taxiPermit`), passenger profile V2, phone verification. Migrations are one-shot on `load()`. **Do not rewrite** — spine only reads via `user.get()` / `user.set(patch)`.                                                          |
