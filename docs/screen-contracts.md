...
| Active-vehicle handling | When the archived id equals the persisted `activeVehicleId` (after trimming both sides), the helper clears `driverGarage.activeVehicleId` to null. It does NOT promote another vehicle to active automatically — the render-time resolver may pick the first remaining non-archived candidate for display, but storage stays explicit. Archiving a non-active vehicle leaves `activeVehicleId` verbatim. |
| Resolver / render filter | `garage.js buildGarageVehicles` filters `archived === true` entries out of the active list **before** the multi-overlay and resolver run. `resolveActiveGarageVehicleId`, `resolveActiveGarageVehicle`, the profile render, the /respond `getUserVehicle`, and the ride_actions accept handoff snapshot therefore never surface an archived vehicle. When the persisted collection contains only archived entries, the builder falls through to the legacy-derived `legacy-1` entry. |
...
| Out of scope | Hard delete (never), restore-promotes-to-active (explicitly forbidden — 05D's make-active handler stays the only writer for `activeVehicleId` outside `appendGarageVehicle({ makeActive: true })`), documents / readiness hook (deferred to 05K), driver-response schema changes, active ride / ride lifecycle / history / receipt writes, backend / API, Mapbox, CSP, inline script-style. |
...
2. **Order Detail remains accessible after accept.** Once `Order.status = 'Заказ принят'`, the screen does NOT redirect to active ride. The primary action becomes **«Открыть поездку»**; the canonical active-ride runtime (`/active-ride`) stays the source of truth for the trip lifecycle.
3. **`DriverOffer` carries its own `expiresAt`.** Suggested default: `expiresAt = min(Order.expiresAt, createdAt + 15 minutes)`. Expired offers transition to `status='expired'` and stop counting as candidates.
