...
| Actions | Cancel: select reason, custom reason, confirm. Problem: select reason, «Отправить» (validates) / «Закрыть». Both: close / Esc / backdrop (cancel disabled mid-loading and on the terminal card). |
