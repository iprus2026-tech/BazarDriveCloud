...
DRIVER_EN_ROUTE              — водитель в пути к точке подачи; en-route
                               ...
                               cancel sheet
DRIVER_APPROACHING_PICKUP    — alias en-route фазы в passenger renderer;
                               рендерится тем же `renderEnRouteSheet`,
                               поэтому кнопка отмены доступна
```
...
Sheet монтируется поверх любого из этих passenger-state. Точка входа
из UI — кнопка/тайл безопасности внутри passenger renderer.
