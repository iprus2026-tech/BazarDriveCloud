...
| Data | With the backend seam enabled for a real non-fallback order, `GET /api/v1/matching/offers?orderId=…` is authoritative — including an empty result — and each selectable server offer carries its canonical `driverId`. With the backend seam off, real driver responses come from `bazardrive.responses.v1` (read-side, BD-RESPOND-ORDER-LINK-02 / #369): `kind==='passenger_response'` rows for the current canonical `orderId`, mapped into the `responses__driver` card shape; the legacy/local fallback can use in-file `MOCK_DRIVERS`. Canonical local order lookup uses `getOrderById()`. |
| Persistence and authority | `/responses` reads `bazardrive.responses.v1` without writing that response store. In the backend-authoritative branch, `POST /api/v1/matching/select` owns the order/offer/assignment/ride transaction; a same-device creator may additionally build a local active-ride mirror. In the backend-off prototype, selection can write `bazardrive.ride_orders.v1` through `acceptOrder()` and `bazardrive.active_ride.v1` through `saveActiveRide()`. These local writes are prototype persistence, not backend authority. |
...
**Status: Contract-only — runtime pending.** This section records the identity and
failure contract for the passenger's «Выбрать водителя» action on `/responses`.
It does not change `responses.js`, activate the backend, add a response status, or
authorize the follow-up runtime slice.
...
| Fail-closed rule | If the clicked card no longer resolves in the current board, its exact `responseId` no longer resolves for this `orderId`, the backend card has no canonical `driverId`, or any resolved identities disagree, the action performs **none** of: `acceptOrder`, `saveActiveRide`, `selectOfferOnBackend`, selection-state persistence, or navigation. It may refresh the board or show an error, but must not substitute `selectedDriver`, `drivers[0]`, the latest response, or a demo driver. |
| State transitions | **Backend-authoritative:** server transaction locks the open order, moves the selected live offer `sent -> accepted`, competing live offers `sent -> rejected`, creates the `ACCEPTED` assignment, moves the order `CREATED -> ACCEPTED`, and bootstraps the ride at `DRIVER_EN_ROUTE`. **Local prototype:** the order moves `CREATED -> ACCEPTED`, the immutable local response record has no selection-status transition, and the ride is seeded at `DRIVER_EN_ROUTE`. This contract does not invent a response status for `bazardrive.responses.v1`. |
...
| Current runtime gap | `responses.js` currently renders `data-driver-id` from the card's generic `driver.id` (which is a local/synthetic `responseId` for both local and server cards), then resolves a click with `drivers.find(...) || selectedDriver || drivers[0]`. That silent fallback can substitute a different driver when the DOM snapshot is stale. The backend branch later passes `driver.driverId`, which is correct only if the intended card was resolved exactly. This paragraph describes the shipped gap; none of it is presented as fixed by 01A. |
| Runtime follow-up (01B) | Resolve the clicked card exactly; validate mode-specific identity before every side effect; remove all selected/first/latest-driver substitution from the select path; keep server `driverId` distinct from local `responseId`; add behavioral negative tests proving stale/foreign/missing identities cause zero API calls, order writes, ride writes, and navigation. Any changed precached `public/src/**` file requires the normal `public/sw.js` monotonic VERSION bump and precache-drift gate. |
| Out of scope | Runtime JavaScript, service worker/CSP, backend route/schema changes, a new response-status model, the separate Order Detail Model-B selection flow, repair of local two-write atomicity, extraction of a shared `acceptOrderAndSeedRide` command, UI redesign, Mapbox, notifications, payment, or history. |
