...
| Builder | `buildGarageVehicles(u, options)` returns `[{ id, model, color, plate, status, source }]`. The legacy vehicle (when `make+model` or either alone produce a usable model line) lands as `{ id: 'legacy-1', status: 'active', source: 'legacy' }`. `?garage=empty` short-circuits to `[]`. `?garage=multi` (only when legacy is present) appends `{ id: 'demo-2', model: 'Kia Rio', color: 'белый', plate: '*** 125', status: 'available', source: 'mock' }` — preview-only, never persisted. |
| Active flag | **Derived UI state only.** `status: 'active'` is computed by the builder on every render from the legacy vehicle. No `activeVehicleId` / `selectedVehicleId` is written to `localStorage`; the driver-response snapshot is unchanged. |
...
| make-active-local state | New non-active per-vehicle button ("Сделать активной") carrying `data-garage-action="make-active"` + `data-garage-state="make-active-local"`. Click is local-feedback flash only ("Сменим, но позже" for 1500ms); no persistence, no router nav. |
...
| Constraints | No persisted `activeVehicleId`, no persisted `selectedVehicleId`, no persisted `garageVehicles[]`, no driver-response snapshot mutation, no active-ride mutation. The smoke (`scripts/smoke-profile-driver-garage.mjs`) blocks all of these patterns in `buildGarageVehicles` / `garageSectionHtml` / `wireGarageActions` bodies and re-asserts the localStorage byte-equality guarantee across all action handlers in both single and multi states. |
| Acceptance | `node scripts/check.mjs` green; smoke covers builder shape (empty / single / multi), per-vehicle DOM hooks, active vs. non-active card variant (badge, status pill vs. make-active button), and the no-mutation guarantee for every handler in the multi state. |
