...
> **Contract-first / partially live (`status: draft`).** This ADR freezes the
> WhatsApp Business Account (WABA) adapter configuration and webhook seam. The
> GET verification challenge is **live**. Message processing (POST) is
> **dark 501** until a separately authorized intake slice is implemented.
...
- WABA is transport. It is not the authority for merchant status, membership,
  quote approval, dispatch, or delivery lifecycle (per 01A invariant 7).
...
- WABA config block is structured-but-dark in `server/src/config.js`.
- GET `/api/v1/webhooks/whatsapp` returns the challenge on a matching token and
  403 on mismatch or empty token.
- POST `/api/v1/webhooks/whatsapp` returns 501 `NOT_IMPLEMENTED`.
- No WABA credential is logged, emitted in metrics, or returned in a response.
- No existing Mini-Yonder service, auth contract, passenger/driver route, or
  PWA behavior is changed.
