...
expect('exports async loadResource(fn, { onRetry, isRetry, fallback = [] })',
  /export\s+async\s+function\s+loadResource\(\s*fn\s*,\s*\{\s*onRetry\s*,\s*isRetry\s*,\s*fallback\s*=\s*\[\s*\]\s*\}\s*=\s*\{\s*\}\s*\)/.test(dataLayer));

// ── C. the contract (loadResource is the only function in this small module,
//        so assertions run against the whole file) ────────────
expect('isRetry shows a non-blocking retrying state first',
  /if\s*\(isRetry\)\s*reportAppShellError\(\s*'retrying'\s*\)/.test(dataLayer));
...
expect('catch reports server_error with the guarded onRetry option',
  /catch[\s\S]*?reportAppShellError\(\s*'server_error'\s*,\s*onRetry\s*\?\s*\{\s*onRetry\s*\}\s*:\s*\{\s*\}\s*\)/.test(dataLayer));
