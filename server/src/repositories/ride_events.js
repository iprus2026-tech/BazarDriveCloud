...
// route returns THAT as the cursor, so `at > $2::timestamptz` round-trips losslessly. A same-µs tie
// between two events is not disambiguated — negligible for separate-transaction status changes;
// revisit with a keyset (at, id) cursor if a denser stream (e.g. chat) later joins this seam.
